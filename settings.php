...
/**
 * Theme Boost Union FF - Settings file
 *
...
if ($hassiteconfig || has_capability('theme/boost_union:configure', context_system::instance())) {
    ...
    if (!$ADMIN->fulltree) {
        // Create Boost Union FF settings page
        // (and allow users with the theme/boost_union:configure capability to access it).
        $tab = new admin_settingpage('theme_boost_union_ff',
    ...
    // phpcs:disable moodle.ControlStructures.ControlSignature.Found
        ...
        require_once($CFG->dirroot . '/theme/boost_union_ff/lib.php');
        require_once($CFG->dirroot . '/theme/boost_union_ff/locallib.php');
        ...
        $tab = new admin_settingpage('theme_boost_union_ff_general',
                get_string('generalsettings', 'theme_boost', null, true));

        // Create inheritance heading.
        $name = 'theme_boost_union_ff/inheritanceheading';
        $title = get_string('inheritanceheading', 'theme_boost_union_ff', null, true);
        ...
        $inheritanceoptions = [
                THEME_BOOST_UNION_CHILD_SETTING_INHERITANCE_INHERIT =>
                        get_string('inheritanceinherit', 'theme_boost_union_ff'),
                THEME_BOOST_UNION_CHILD_SETTING_INHERITANCE_DUPLICATE =>
                        get_string('inheritanceduplicate', 'theme_boost_union_ff'),
        ];

        // Setting: Pre SCSS inheritance setting.
        $name = 'theme_boost_union_ff/prescssinheritance';
        $title = get_string('prescssinheritancesetting', 'theme_boost_union_ff', null, true);
        $description = get_string('prescssinheritancesetting_desc', 'theme_boost_union_ff', null, true).'<br />'.
                get_string('inheritanceoptionsexplanation', 'theme_boost_union_ff', null, true);
        ...
        $name = 'theme_boost_union_ff/extrascssinheritance';
        $title = get_string('extrascssinheritancesetting', 'theme_boost_union_ff', null, true);
        $description = get_string('extrascssinheritancesetting_desc', 'theme_boost_union_ff', null, true).'<br />'.
                get_string('inheritanceoptionsexplanation', 'theme_boost_union_ff', null, true);
        $setting = new admin_setting_configselect($name, $title, $description,
                THEME_BOOST_UNION_CHILD_SETTING_INHERITANCE_INHERIT, $inheritanceoptions);
        $setting->set_updatedcallback('theme_reset_all_caches');
        ...
        /**********************************************************
         * EXTENSION POINT:
         * Add your Boost Union FF settings here.
         *********************************************************/
