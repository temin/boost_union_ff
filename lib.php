 ...
 * @return string
 */
function theme_boost_union_ff_get_main_scss_content($theme) {
    global $CFG;
 ...
 * @return string
 */
function theme_boost_union_ff_get_pre_scss($theme) {
    global $CFG;
    ...
    // If such a fault happens, the Boost Union FF administrator can switch the inheritance to 'Duplicate'.
    // This way, we will add the pre SCSS code with the explicit use of the Boost Union configuration to the stack.
    $inheritanceconfig = get_config('theme_boost_union_ff', 'prescssinheritance');
    ...
    // And add Boost Union FF's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_ff/scss/pre.scss');
 ...
 * @return string
 */
function theme_boost_union_ff_get_extra_scss($theme) {
    global $CFG;
    ...
    $inheritanceconfig = get_config('theme_boost_union_ff', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_CHILD_SETTING_INHERITANCE_DUPLICATE) {
...
/**
 * Callback function for theme_boost_union to allow Boost Union FF to add cards to the Boost Union settings overview page.
 * This function is expected to return an array of arrays containing values with the keys 'label', 'desc', 'btn' and 'url'.
 *
 * @return array
 */
function theme_boost_union_ff_extend_busettingsoverview() {
