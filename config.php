...
/**
 * Theme Boost Union FF - Theme config
 *
...
// This move will save us from duplicating all lines from Boost Union's config.php into Boost Union FF's config.php.
...
// Then, we require Boost Union FF's locallib.php to make sure that it's always loaded.
require_once($CFG->dirroot . '/theme/boost_union_ff/locallib.php');

// Next, we overwrite only the settings which differ between Boost Union and Boost Union FF.
$THEME->name = 'boost_union_ff';
$THEME->scss = function($theme) {
    return theme_boost_union_ff_get_main_scss_content($theme);
};
$THEME->parents = ['boost_union', 'boost'];
$THEME->extrascsscallback = 'theme_boost_union_ff_get_extra_scss';
$THEME->prescsscallback = 'theme_boost_union_ff_get_pre_scss';
...
// Lastly, we replicate some settings from Boost Union at runtime into Boost Union FF's settings.
...
// it would be quite ugly to duplicate all of these settings again to Boost Union FF.
